  ...
  <div class="component-wrapper container">
    ...
    <div v-if="book" >
      ...
      <div class="book-wrapper" v-if="book != null">        
        <h1>{{ book.title }}</h1>
        <p><strong>Category: </strong>: <a href="#" v-on:click.prevent="navigateTo(`/front?search=${book.category}`)">{{ book.category }}</a></p>      
        <div class="content" v-html="book.content"></div>
        <!-- <p>category: {{ book.category }}</p>
        <p>status: {{ book.status }}</p> -->    
      </div>
      <div class="back-nav"><button class="btn btn-success" v-on:click="navigateTo('/front-books')"><i class="fas fa-arrow-left"></i> Back..</button></div>            
      <br>
    </div>
  </div>
</template>
